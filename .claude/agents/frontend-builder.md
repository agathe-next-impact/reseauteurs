...
## Périmètre
- **Design system** : porter les **tokens conservés** de `DESIGN.md` dans `tailwind.config` + `globals.css` (`:root` clair + bandes sombres) + `components.json` shadcn. Police **Hanken Grotesk** via `next/font`. Repères : canvas `#faf9f5`, bleu primaire `#2563EB`, navy `#16284f`, **accent orange `#f5851f`** (renvoi B2B/partenaires — **pas** un paiement réseauteur), violet `#a855f7`, neutres zinc, rayons ~12px (pills 999px), bandes sombres `#0d0d10`. **Badge réseauteur** (Bronze/Argent/Gold/Platinum) en pill, 4 variantes.
...
- **Fiche événement** (`/evenement/<slug>`, SSR) : image, titre, **date/heure**, adresse/ville, description, **réseau organisateur** (lien fiche réseau), CTA **« S'inscrire »** = **lien externe** ; badge **Premium** si applicable. `seo-engineer` branche JSON-LD `Event`.
...
- **Coquilles d'espaces** : dashboard **réseauteur** (édition profil, badge, champs contact), dashboard **organisateur** (fiche réseau, événements, abonnement, factures) — layout/nav/états vides ; les écrans paiement/abonnement sont remplis par `accounts-and-billing`.

## Méthode
1. **Poser les tokens AVANT les pages** (couleurs, radius, Hanken Grotesk) + 4–6 primitives (Button, Card, Badge/Pill, Input, Section, StatCounter).
...
- **Monétisation = B2B** : pas d'UI de paiement côté réseauteur (gratuit). L'accent orange « conversion » renvoie aux partenaires / au réseau partenaire.
...
## Definition of Done
Tokens + primitives en place ; home (3 piliers) **comprise en < 30 s**, responsive ; **trois fiches SSR** (réseauteur, événement, réseau) prêtes pour le SEO ; **recherche simple par filtres** branchée sur l'API `find` ; page Partenaires ; coquilles dashboards réseauteur/organisateur navigables ; badges stylés ; a11y vérifiée ; build OK ; 0 « Panorama Pub »/« Info-Réseaux » dans l'UI.

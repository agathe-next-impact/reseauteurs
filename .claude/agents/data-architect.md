...
titre, **slug**, description, **date / heure**, adresse, ville, département, image, **organisateur = réseau XOR réseauteur Plus** (`reseau` N-1 vers `reseaux` **ou** `organisateurReseauteur` vers `reseauteurs`, invariant serveur), **lien d'inscription externe** (`lienInscription` URL), **type** (relation `types_evenement`), `gratuit`/`tarif`, **`geom`**, `creeParUser` (propriété), statut. **Retirer** de l'ancien modèle : `participer`/`participantsSignales`, quota, `serieId`/récurrence. ⚠️ Le **drapeau `premium` est SUPPRIMÉ** (ADR-0012, colonnes droppées). Deux régimes d'inscription (ADR-0013) : événement **de réseau** → lien externe ; événement **de réseauteur Plus** → **inscription en ligne** (collection `inscriptions`, liste gérée par l'organisateur).

### `reseaux` (à conserver comme ENTITÉ + taxonomie)
nom, **slug**, logo, description, présentation, lien internet, **`niveau`** (4 valeurs `local`/`regional`/`national`/`international` — tête = non-local), **`parent`** (hiérarchie tête↔local, 2 étages, ADR-0014), **compteurs dérivés** (`nbReseauteurs`, `nbEvenements`, recalculés par hook), **drapeau `partenaire`** + **`palier`** (`fiche`/`starter`/`growth`/`enterprise`) + `partenaireExpireAt` (abonnement porté par la **tête**, posé par `accounts-and-billing` via webhook), **`statut`** (`publiee`/`suspendue` — une tête revendiquée naît `suspendue`, publiée par l'abonnement), `source` (`revendique`/`importe`), `seoField`. Possédé par **0..1 compte** (`organisateur` pour une tête ; un **réseauteur Plus** possède jusqu'à 3 locaux) ; peut exister sans propriétaire (importé, revendiquable). Sert **à la fois** de fiche-entité **et** de cible de la relation M2M des réseauteurs et de la relation N-1 des événements.
...
### `users` (4 rôles)
enum `role` → **`reseauteur` / `organisateur` / `partenaire` / `admin`** (retrait `fournisseur` ; l'enum `plan` 3-paliers reste **dormant** en attendant sa suppression). Champs **Plus** (ADR-0013) : `plusActif` / `plusSource` (`abonnement` ou `licence` legacy) / `plusExpireAt` / `stripeSubscriptionId`, posés par le webhook. Hooks signup → **auto-création** selon le rôle : `reseauteur` (statut « en attente »), `partenaires` (statut `expire`), `reseaux` national (statut `suspendue`, source `revendique`). Conserver auth/Stripe/onboarding/blacklist ; champs `groupe`/`pendingGroupeCode` + `plusLicencePack` **dormants** (ADR-0009/0015).
...
4. Seed dev minimal : quelques réseauteurs réalistes par grandes villes, quelques réseaux (BNI/DCF…), quelques événements (dont 1 organisé par un réseauteur Plus) — cartes non vides en local.

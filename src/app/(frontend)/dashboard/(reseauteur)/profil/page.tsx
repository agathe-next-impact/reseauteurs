      ...
      {!reseauteur ? (
        ...
        <div className="bg-white rounded-2xl border border-dashed border-[#e4e4e7] p-10 text-center">
          ...
          <p className="text-sm text-[#71717a] mb-6">
            Votre profil réseauteur est créé automatiquement. Rechargez la page dans quelques secondes.
          </p>
        </div>
      ) : (
          ...
          <div className="bg-white rounded-2xl border border-[#e4e4e7] p-6">
            <div className="flex items-start gap-4 mb-5">
              {photoUrl ? (
                ...
                <div className="w-16 h-16 rounded-xl bg-[#bfdbfe]/30 flex items-center justify-center text-[#2563EB] font-bold text-xl shrink-0" aria-hidden>
                  {reseauteur.prenom?.charAt(0)}{reseauteur.nom?.charAt(0)}
                </div>
              )}
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <p className="font-bold text-[#16284f]">{reseauteur.prenom} {reseauteur.nom}</p>
                  {reseauteur.badge && <BadgeReseauteur badge={reseauteur.badge} />}
                </div>
                {reseauteur.fonction && <p className="text-sm text-[#52525b]">{reseauteur.fonction}</p>}
                {reseauteur.entreprise && <p className="text-sm text-[#71717a]">{reseauteur.entreprise}</p>}
                {reseauteur.ville && (
              ...
              <div className={`shrink-0 text-xs font-medium px-2 py-1 rounded-full ${
                ...
                {reseauteur.statut === 'valide' ? 'Validé' : reseauteur.statut === 'suspendu' ? 'Suspendu' : 'En attente'}
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-[#e4e4e7]">
              ...
              <div className="grid grid-cols-2 gap-2">
                <FieldPreview icon={<Phone size={12} />} label="Téléphone" value={reseauteur.telephone} muted="Non partagé" />
                <FieldPreview icon={<Mail size={12} />} label="Email public" value={reseauteur.emailContact} muted="Non partagé" />
                <FieldPreview icon={<Globe size={12} />} label="Site web" value={reseauteur.site} muted="Non renseigné" />
                <FieldPreview icon={<ExternalLink size={12} />} label="LinkedIn" value={reseauteur.linkedin ? 'Renseigné' : undefined} muted="Non renseigné" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#e4e4e7] p-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <p className="text-xs text-[#71717a]">
                Basé sur votre déclaration : {reseauteur.evenementsParMois ?? 0} événement{(reseauteur.evenementsParMois ?? 0) > 1 ? 's' : ''}/mois
              </p>
            </div>
          </div>
...
function FieldPreview({ icon, label, value, muted }: {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <span className="text-[#a1a1aa]" aria-hidden>{icon}</span>

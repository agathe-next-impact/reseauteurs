...
import { SOCIAL_COLORS, SOCIAL_LABELS, socialHandle, type SocialPlatform, type SocialLinkData } from '@/lib/socials'

function SocialIcon({ platform, size = 14 }: { platform: SocialPlatform; size?: number }) {
  const color = SOCIAL_COLORS[platform]
  const props = { width: size, height: size, viewBox: '0 0 24 24', fill: color, className: 'shrink-0' }
...
export function SocialLinks({ links }: SocialLinksProps) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {links.map((social, i) => {
        const label = SOCIAL_LABELS[social.plateforme]
        return (
          <a
            key={i}
            href={social.url}
